...
To obtain binary run-time statistics information, call the uxTaskGetSystemState() API function. To obtain
...
Run-time statistics need to measure fractions of a tick period. Therefore, the RTOS tick count is not used as the
...
combining the RTOS tick count with the current value of an ARM Cortex-M SysTick timer. Some of the demo
...
intended to be included in the RTOS port layer, which is why the macros are prefixed 'port', but it has proven
...
The TaskStatus_t structure is shown in Listing 12.5, and the TaskStatus_t structure members are
...
The second column is the task's state, where 'X' means Running, 'R' means Ready, 'B' means Blocked, 'S'
...
Output detailed execution sequencing information by any means available on the target hardware. Trace
...
In accordance with software engineering best practice, FreeRTOS maintains a strict data hiding policy. Trace
